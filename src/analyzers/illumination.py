...
class IlluminationAnalyzer:
    ...
    def __init__(
        ...
        hue_tol=12,
        # floor ROI gating: cast shadows of interest live on the floor;
        # window mullions / frames / railings live above the horizon.
        # Restricting candidates to the floor band removes almost all
        # of that structural false-positive class in one step.
    ...
    def build_floor_roi_mask(self, shape):
        """Binary mask that is 255 only in the bottom band of the image
        (the floor plane, roughly). Cast shadows we care about live
        here; window mullions, railings, and door/frame edges live
        above the horizon and are excluded outright, regardless of how
        similar they look in local contrast to a real shadow stripe."""
        h, w = shape[:2]
